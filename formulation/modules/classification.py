...
def determine_new_accuracy(N, X_data, Y_data):
    """
    ...
    Classicifation = RFC.fit(x_train, y_train)
    y_pred = Classicifation.predict(x_test)
    CFI = Classicifation.feature_importances_
    importance_d = pd.DataFrame(CFI, index=list(X_data),
                                columns=['Value']).sort_values(by=['Value'],
                                                               ascending=False)
    indexlist = []
    for i in range(N):
        indexlist.append(importance_d.index[i])
